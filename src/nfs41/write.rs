...
impl Mount41 {
    ...
    async fn mds_write(&self, fh: &Bytes, offset: u64, data: Bytes) -> Result<u32> {
        let sid = self.state.has_open(fh, AccessMode::Write).await
            .unwrap_or_else(StateId::anonymous);
        let stateid = sid.raw;
        let data_len = data.len() as u32;
        let fh_ref = fh.clone();
        let resp = self.compound_write("write", data, |b| {
            b.putfh(&fh_ref).write_header(&stateid, offset, 2 /* FILE_SYNC4 */, data_len)
        }).await?;
        resp.op_ok(1)?; // PUTFH
    ...
    pub(crate) async fn create(&self, dir_fh: Bytes, filename: &str, mode: Option<u32>) -> Result<mount::ObjRes> {
        ...
        let resp = self.compound("create", |b| {
            b.putfh(&dir_fh).open(&open_args).getfh().getattr(&bitmap)
        }).await?;
        resp.op_ok(1)?; // PUTFH
        let open_op = resp.op_ok(2)?; // OPEN
        ...
        if let Some(m) = mode {
            ...
            if let Err(e) = self.compound("setattr", |b| {
        ...
        // Close the open stateid; log but don't fail — file was created successfully.
        if let Err(e) = self.compound("close", |b| b.putfh(&fh).close(0, &stateid)).await {
            warn!(error = %e, "create: CLOSE failed — open stateid will leak until server lease expiry");
        }
        Ok(mount::ObjRes { fh, attr: Some(attr) })
    }
    ...
    pub(crate) async fn mkdir(&self, dir_fh: Bytes, dirname: &str, mode: u32) -> Result<mount::ObjRes> {
        // 不在 CREATE createattrs 传 mode，避免 NFS4ERR_ATTRNOTSUPP；创建后 SETATTR 设置。
        let bitmap = standard_getattr_bitmap();
        let resp = self.compound("mkdir", |b| {
            b.putfh(&dir_fh)
        ...
        if let Err(e) = self.setattr(fh.clone(), None, Some(mode), None, None, None, None, None).await {
