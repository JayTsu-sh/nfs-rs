...
timeout 300 cargo test --locked --test lab_e2e \
  nfs_v41_session_fault_reopen_resume_checksum -- --ignored --nocapture &
test_pid=$!
...
[[ -e "$ready" ]] || {
  echo "recovery test did not become ready" >&2
  exit 1
}

ssh_lab "$fault_host" \
  "sudo -n /usr/local/sbin/terrasync-lab-nfs-fault apply-session-fault '$run_id'"
: >"$done_file"
wait "$test_pid"
